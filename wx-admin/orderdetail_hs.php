...
if (!$_SESSION['openid']||$_SESSION['user_level'] <2 ) {
	...
	if (isset($_GET["sp"])) {
        $urow = array();
        $urow['service_progress'] = $_GET["sp"];
        $upstate = serviceupdate($db, $urow, $order_ID);
		if($upstate == "True") {
			$scp = new template_message();
			$scp->send_customer_progress($order_ID, $openid, $workername, $type, $address, '房管家第'.$_GET["sp"]."服务");
			$scp->send_manager_progress($order_ID, $aopenid, $workername, $type, $address, '房管家第'.$_GET["sp"]."服务");
			echo "<script>window.location.href='orderdetail_hs.php?order_ID=$order_ID';</script>"; 
		}
    }
...
?>
    ...
    <ul class="table-view table-view2" id="onup"> 
     ...
     <li class="table-view-cell table-disabled" id="numtable0"> 
      ...
      <div class="table-cell" style="text-align:left; ">
       共<?php echo $s_detailfrq;?>次服务，已完成<?php echo $service_progress;?>次。
       <input name="s_progress_up" id="s_progress_up" class="upsp_btn" type="button" onClick="javascript:upsp(<?php echo (int)$service_progress+1;?>)" value="完成第<?php echo (int)$service_progress+1;?>次">
      </div> 
     </li>
  ...
  function upsp(s) {
	  if(confirm("确认更新进度？"))
		  {
			  location.href='?sp='+s+'&order_ID=<?php echo $order_ID?>';
		   }
		  else
		  {
			  return false;
