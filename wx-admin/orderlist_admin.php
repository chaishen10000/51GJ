...
if (!$_SESSION['openid']||$_SESSION['user_level'] <2 ) {
	...
	$sql = "";
	if($getstate == 1) $sql = "state='已提交'";
	if($getstate == 2) $sql = "state='服务中'";
	if($getstate == 3) $sql = "state='已服务'";
	if($getstate == 4) $sql = "state='已完成'";
	//获取任务列表
	$row = $db->row_select('wx_order', $sql, 0, '*', 'order_ID');
	$strtemp = "";
	if(count($row)==0){
		$strtemp = "<br/><br/><br/><h4 style='color:#CCC'>任务为空！</h4>";
	}else{
	foreach ($row as $key => $value) {
		...
		$strtemp.= "<a href='/wx-admin/orderdetail.php?order_ID=".$order_ID."' data-ignore='push'> ";
		$strtemp.= $type;
		$strtemp.= "</a></h4></td><td ><h4>";
		if($state=="已取消"){
		   $strtemp.= "(".$state.")";
		}
		$strtemp.= "</h4></td><td >#".$order_ID."</td></tr><tr height='30' align='center'><td colspan='2'>";
		$strtemp.= $time_serv;
		$strtemp.= "</td><td >";
		$strtemp.= $cash_fee."元";
...
<!--bar-tab begin -->
	<div class="bar bar-footer bar-tab">
        ...
        <a class="tab-item <?php if($getstate==4) echo 'active'?>" href="?state=4" data-ignore="push">
	        <span class="tab-label">已审核</span>
	    </a>
	</div>
    <!--bar-tab end-->
</body>
